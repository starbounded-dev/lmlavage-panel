    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <Card>
          ...
          <CardContent>
            <Table><TableHeader><TableRow><TableHead>Nom</TableHead><TableHead>Type</TableHead><TableHead className="text-right">Vente Alexis</TableHead><TableHead className="text-right">Vente Guillaume</TableHead><TableHead className="text-right">Vente P-O</TableHead><TableHead className="text-right">Split A+G</TableHead><TableHead className="text-right">Historique</TableHead><TableHead className="text-right">Montant historique</TableHead></TableRow></TableHeader>
              <TableBody>{data.allocationBuckets.map((bucket) => <TableRow key={bucket.id}><TableCell className="font-medium">{bucket.name}</TableCell><TableCell><Badge variant="secondary">{bucket.type === "person" ? "Personne" : "Réserve"}</Badge></TableCell><TableCell className="text-right font-mono">{bucket.alexisSalePercentage}%</TableCell><TableCell className="text-right font-mono">{bucket.guillaumeSalePercentage}%</TableCell><TableCell className="text-right font-mono">{bucket.poSalePercentage}%</TableCell><TableCell className="text-right font-mono">{bucket.splitSalePercentage}%</TableCell><TableCell className="text-right font-mono">{bucket.percentage}%</TableCell><TableCell className="text-right font-mono">{formatCad(bucket.amount)}</TableCell></TableRow>)}</TableBody>
            </Table>
          </CardContent>
        </Card>
      </div>
